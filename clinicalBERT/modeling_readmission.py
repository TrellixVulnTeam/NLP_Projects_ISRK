...
class BertForSequenceClassification_3layer(PreTrainedBertModel):
    """BERT model for classification.
    This module is composed of the BERT model with a linear layer on top of
    the pooled output.
    ...
    def __init__(self, config, num_labels):
        super(BertForSequenceClassification_3layer, self).__init__(config)
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier1 = nn.Linear(config.hidden_size, 2048)
        self.classifier2 = nn.Linear(2048, 768)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None):
        ...
        pooled_output2 = self.dropout(pooled_output)
        classifier_ouput1 = self.classifier1(pooled_output2)
        classifier_ouput2 = self.classifier2(classifier_ouput1)
        logits = self.classifier3(classifier_ouput2)
